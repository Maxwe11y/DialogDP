...
def load_data(filename, map_relations):
    ...
    for dialog in data:
        ...
        for edu in dialog["edus"]:
            ...
            edu["turn"] = turn
        have_relation = {}
        have_y = {}  # used for single parent setting
        relations = []
        ...
        for relation in dialog["relations"]:
            if (relation["x"], relation["y"]) in have_relation:
                continue
            # if relation["y"] in have_y:  # used for single parent setting
            #     if relation["type"] not in count_omit.keys():
            #         count_omit[relation["type"]] = 1
            #     else:
            #         count_omit[relation["type"]] += 1
            #     continue
            relations.append(relation)
            have_relation[(relation["x"], relation["y"])] = True
            # have_y[relation["y"]] = True # used for single parent setting
        ...
        #dialog["relations"] = sorted(dialog["relations"], key=lambda a, b: cmp_relation(a, b))
        ...
        for i in range(len(dialog["edus"])):
            if cnt[i] > 1:
                cnt_multi_parents += 1
    f_in.close()
    # for r in count_omit.keys():
    #     count_omit_integer[map_relations[r]] = count_omit[r]
    ...
    # with open('./visual/count_omit_integer.json', 'w') as f:
    #     json.dump(count_omit_integer, f)

    return data


def load_data_molweni(filename, map_relations):
    print("Loading data:", filename)
    f_in = open(filename)
    #inp = f_in.readline()
    #data = json.loads(inp)
    ...
    # with open('./visual/count_omit_integer_molweni.json', 'w') as f:
    #     json.dump(count_omit_integer, f)
...
def build_vocab(data):
    ...
    for i, word in enumerate(vocab_list):
        ...
        if word in vectors:
            ...
            embed.append(list(np.zeros(FLAGS.dim_embed_word, dtype=np.float32)))

    embed = np.array(embed, dtype=np.float32)
    embed_relation_dim = FLAGS.dim_embed_relation
    embedding_relation = [list(np.zeros(embed_relation_dim))]
    embedding_relation.extend([list(np.random.normal(loc=0.0, scale=0.1, size=embed_relation_dim)) for _ in range(FLAGS.num_relations)])
    relation_embed = np.array(embedding_relation)

    word_idx = dict((c, k) for k, c in enumerate(vocab_list_major))
    word_idx_rev = dict((k, c) for k, c in enumerate(vocab_list_major))
    print("Pre-trained vectors: %d/%d" % (cnt_pretrained, len(embed)))
    #return vocab_list_major, embed
    return word_idx_rev, word_idx, embed, relation_embed


# def preview_data(data, pred, map_relations, vocab, idx, raw_data, id_dict):
def preview_data(data, pred, map_relations, vocab, idx):
    ...
    for i, dialog in enumerate(data):
        ...
        for relation in dialog["relations"]:
            print
            relation["x"], relation["y"], map_relations_inv[relation["type"]]
        print
        "predicted:"
        # raw_data[id_dict[dialog['id']]]['relations'] = []
        for relation in pred[i]:
            # raw_data[id_dict[dialog['id']]]['relations'].append({'y': relation[1], 'x': relation[0], 'type': map_relations_inv[relation[2]], 'type_num': relation[2]})
            print
            relation[0], relation[1], map_relations_inv[relation[2]]
